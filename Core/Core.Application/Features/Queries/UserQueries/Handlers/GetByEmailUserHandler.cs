...
namespace Core.Application.Features.Queries.BusinessPlaceQueries.Handlers
{
    public class GetByEmailUserHandler : IRequestHandler<GetByEmailUserQuery, IResultDataDto<UserDto>>
    {
        ...
        public async Task<IResultDataDto<UserDto>> Handle(GetByEmailUserQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var repoResult = _userRepository.GetSingle(predicate: d => d.Email == request.Email, include: d => d.Include(c => c.Client).Include(r=>r.UserRole));
                if (repoResult == null) return result.SetStatus(false).SetErrorMessage("Not Found Data").SetMessage("Your account was not found! If you think there is a mistake, please contact the support team.");
                if(repoResult.IsEnable == false) return result.SetStatus(false).SetErrorMessage("Not Found Data").SetMessage("Your account is not enable. Please contact the support team.");
